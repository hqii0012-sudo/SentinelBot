...
def system_page():
    """返回系统管理页面"""
    return send_from_directory(FRONTEND_DIR, 'system.html')

@app.route('/related-party')
def related_party_page():
    """返回关联交易页面"""
    return send_from_directory(FRONTEND_DIR, 'related-party.html')

@app.route('/shadow-shareholder')
def shadow_shareholder_page():
    """返回影子股东识别页面"""
    return send_from_directory(FRONTEND_DIR, 'shadow-shareholder.html')

@app.route('/micro-power')
def micro_power_page():
    """返回微权力清单与拦截页面"""
    return send_from_directory(FRONTEND_DIR, 'micro-power.html')
...
def ask():
    ...
    try:
        ...
        result = ai_provider.ask(query, conversation_id)
